...
<div class="dvizh_order_form">
    <?php $form = ActiveForm::begin(['action' => Url::toRoute(['/order/order/create'])]); ?>
        ...
        <div class="row">
            <div class="col-lg-6"><?= $form->field($orderModel, 'client_name')->textInput(['value' => Yii::$app->user->identity->profile->firstname . ' ' . Yii::$app->user->identity->profile->lastname]) ?></div>
            <div class="col-lg-3"><?= $form->field($orderModel, 'phone')->textInput() ?></div>
            <div class="col-lg-3"><?= $form->field($orderModel, 'email')->textInput(['value' => Yii::$app->user->identity->email]) ?></div>
        </div>
